...
async function defaultGenerate({ brief, prior, fixes, rubric }) {
  // H1: make the generator rubric-aware — it writes TOWARD the criteria from
  // round one, not just reacting to fixes after the fact.
  const criteria = rubric
    ? `\n\nThe artifact will be scored on these criteria — optimise for all of them:\n${rubric.criteria.map((c) => `- ${c.name}: ${c.guide}`).join("\n")}`
    : "";
  const SYSTEM = `You produce a single ARTIFACT for the BRIEF. Be specific and concrete; use real details from the brief; cut filler and buzzwords. If a PRIOR ARTIFACT and FIXES are given, revise to address EVERY fix while keeping what already works.${criteria}\nOutput ONLY the artifact itself — no preamble, no commentary, no markdown fences.`;
  const user = prior
    ? `BRIEF: ${brief}\n\nPRIOR ARTIFACT:\n${prior}\n\nFIXES TO ADDRESS:\n${fixes.map((f) => `- ${f}`).join("\n")}`
    : `BRIEF: ${brief}`;
...
export async function iterate({
  ...
  const rounds = [];
  let artifact = await generate({ brief, prior: null, fixes: null, rubric });
  let best = null;
  ...
  for (let r = 1; r <= maxRounds; r++) {
    ...
    artifact = await generate({ brief, prior: artifact, fixes, rubric });
  }
